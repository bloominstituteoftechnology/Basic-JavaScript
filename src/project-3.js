...
const makeCat = (name, age) => {
  ...
  // return the object
  const cat = { name, age, meow: () => 'Meow!' }; return cat;
};

const addProperty = (object, property) => {
  object[property] = null; return object;
};
...
const deleteProperty = (object, property) => {
  delete object[property]; return object;
};
  // remove the property from the object
  // return the object

const newUser = (name, email, password) => {
  const obj = { name, email, password }; return obj;
};
...
const verifyPassword = (user, password) => {
  if (user.password === password) return true; return false;
};
...
const setUsersToPremium = (users) => {
  users.forEach(el => el.isPremium = true); return users;
};
...
const sumUserPostLikes = (user) => {
  let sum = 0;
  user.posts.forEach(obj => sum += obj.likes); return sum;
};
