...
const setUsersToPremium = (users) => {
  ...
  // users is an array of objects, each object numbered 0 to users.length - 1
  // in each of these objects is a property called 'isPremium'
  // iterate over each object in users array and set the property to isPremium = true
  // users = [ {object1}, {object2}] e.g. users[0] = {object1}/
  // users[0].isPremuim
...
const sumUserPostLikes = (user) => {
  ...
  // return the sum
  // user --> posts --> post --> likes   user.posts[0].likes = 2, user.posts[1].likes= 0,
  let sum = 0;
