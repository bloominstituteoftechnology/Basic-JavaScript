...
return user.password === password;
  ...
  return users.map((user) => { user.isPremium = true; return user; });
  };

  const sumUserPostLikes = (user) => {
 @@ -75,6 +91,9 @@ const sumUserPostLikes = (user) => {
 ...
 @@ -87,6 +106,9 @@ const addCalculateDiscountPriceMethod = (storeItem) => {
    // discountPrice = 20 - (20 * .2)
    // Make sure you return storeItem after adding the method to it
    // hint: arrow functions don't bind a this
  storeItem.calculateDiscountPrice = () => storeItem.price * (1 - storeItem.discountPercentage);
